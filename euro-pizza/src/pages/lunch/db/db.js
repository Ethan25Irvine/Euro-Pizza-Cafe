const Lunch = [
    {
        header: "Lunch Selections",
        food: [
            {
                key: 1,
                name: "",
                price: "",
                description: "Lunch includes these items & anything from the Entrèes menu\nServed from 11a.m to 4p.m."
            },
            {
                key: 2,
                name: "Pizza by the slice (until 3:00)Ⓥ",
                price: "2.5",
                description: "\nAdd choice of toppings from pizza menu for 50¢ each"
            },{
                key: 3,
                name: "Spaghetti with Homemade Marinara SauceⓋ",
                price: "8",
                description: "\nAdd Meatballs or Meat Sauce for $2"
            },{
                key: 4,
                name: "Soup & Salad",
                price: "8",
                description: "Bowl of soup and side salad"
            },{
                key: 5,
                name: "Veggie PiadinaⓋ",
                price: "10",
                description: "Sautéed veggies on top of flatbread with a side of hummus\nAdd Chicken or Gyro Meat for $4"
            },{
                key: 6,
                name: "Strawberry Piadina",
                price: "12",
                description: "Flatbread with spinach, red onion, diced tomatoes, walnuts, cranberries, blue cheese crumbles, topped with zesty orange vinaigrette dressing\nAdd Avocado for $2"
            },{
                key: 7,
                name: "Chicken Avocado Piadina",
                price: "13",
                description: "Flatbread topped with diced chicken, red onion, avocado, bell peppers, tomato, spinach, feta cheese, zesty orange vinaigrette dressing"
            },{
                key: 8,
                name: "Pasta PrimaveraⓋ",
                price: "11",
                description: ""
            },{
                key: 9,
                name: "Fettucinne AlfredoⓋ",
                price: "11",
                description: "\nAdd Chicken for $4 or Shrimp for $6"
            },{
                key: 10,
                name: "Cheeseburger",
                price: "11",
                description: "All beef or turkey patty cooked to order with provolone or American cheese, onion, lettuce and tomatoes; mayo or mustard upon request. Served with side of fries."
            },{
                key: 11,
                name: "Chipotle Ranch Burger",
                price: "13",
                description: "All beef or turkey topped with cheddar cheese, tomatoes, lettuce, chipotle-ranch dressing, topped with grilled onions and mushrooms. Served with side of fries."
            }
            

        ]
    },
    {
        header: "Sandwiches",
        food: [
            {
                key: 1,
                name: "",
                price: "",
                description: "\nServed on pita or french roll with fries or cup of soup"
            },
            {
                key: 2,
                name: "Italian Combo",
                price: "11",
                description: "Genoa Salami, Ham, Bacon and Fresh Provolone Cheese."
            },{
                key: 3,
                name: "Caprese on PitaⓋ",
                price: "10",
                description: "Fresh Mozzarella, tomatoes, fresh basil, red onions, olive oil, black pepper and balsamic glaze\nAdd Chicken or Gyro Meat for $3"
            },{
                key: 4,
                name: "Meatball Parmigiana",
                price: "11",
                description: ""
            },{
                key: 5,
                name: "Greek Wrap",
                price: "11",
                description: "Fresh spinach, red onion, sliced tomatoes, cucumber, feta cheese, tzatziki sauce\nAdd Chicken or Lamb Gyro for $3"
            },{
                key: 6,
                name: "Gyro",
                price: "11",
                description: "Gyro meat, onions, tomatoes, and tzatziki sauce, served on pita bread."
            },{
                key: 7,
                name: "Philly Steak Sandwich",
                price: "11",
                description: "Philly meat, peppers, mushrooms, and onions with melted mozzarella cheese."
            },{
                key: 8,
                name: "California Chicken Sandwich",
                price: "11",
                description: "Chicken breast with melted mozzarella, lettuce, tomato, fresh sliced avocado and chipotle-mayo spread served on a brioche bun."
            },{
                key: 9,
                name: "Chicken BBQ Sandwich",
                price: "11",
                description: "Grilled chicken breast with BBQ sauce, melted mozzarella cheese, topped with caramelized onion, cilantro, served on a brioche bun."
            }

        ]
    },
    {
        header: "Side Dishes",
        food: [
            {
                key: 1,
                name: "6 Bread Knots with side of marinara sauce",
                price: "5",
                description: ""
            },
            {
                key: 2,
                name: "Pita",
                price: "3",
                description: ""
            },
            {
                key: 3,
                name: "Marinara",
                price: "2",
                description: ""
            },
            {
                key: 4,
                name: "Alfredo Sauce",
                price: "2.5",
                description: ""
            },
            {
                key: 5,
                name: "Pesto Sauce",
                price: "3",
                description: ""
            },
            {
                key: 6,
                name: "French Fries",
                price: "3",
                description: ""
            },
            {
                key: 7,
                name: "Sweet Potato Fries",
                price: "4",
                description: ""
            },
            {
                key: 8,
                name: "Meatballs",
                price: "6",
                description: ""
            },
            {
                key: 9,
                name: "Rice",
                price: "4",
                description: ""
            }

        ]
    }
]

export default Lunch;